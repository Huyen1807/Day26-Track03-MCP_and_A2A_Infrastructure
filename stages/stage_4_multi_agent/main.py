...
async def check_routing(state: LegalState) -> dict:
    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves data, privacy, GDPR, data breach, CCPA'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
    ...
    # Deterministic privacy routing: only trigger privacy specialist when
    # explicit privacy/data keywords appear in the question text.
    question_lower = state["question"].lower()
    needs_privacy = any(
